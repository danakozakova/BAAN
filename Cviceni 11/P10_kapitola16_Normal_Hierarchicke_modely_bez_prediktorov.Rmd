...
ppc_intervals(artist_means$popularity, 
  ...
  scale_x_continuous(labels = artist_means$artist,
                     breaks = 1:nrow(artist_means)) +
  xaxis_text(angle = 90, hjust = 1) +
  labs(title = "No pooled model: Predikcie centrovane na pozorovanom priemere - predikčný model pre každého umelca",
       subtitle = "Tmavé body = pozorované priemery, svetlé intervaly = posteriori prediktívne intervaly") +
  theme_minimal()
```
...
$$
\begin{array}{lrl}
\text{Vrstva 1:} & Y_{ij} | \mu_j, \sigma_y & \sim \text{model variability v rámci (WITHIN) modelu = VNÚTRI umelca } j \\
\text{Vrstva 2:} & \mu_j | \mu, \sigma_\mu & \sim \text{model variability MEDZI (BETWEEN) umelcami}\\
\text{Vrstva 3:} & \mu, \sigma_y, \sigma_\mu & \sim \text{priori modely pre globálne parametre}
\end{array}
$$
$\sigma_mu$ je variabilita spoločná
$\sigma_y$ je variabilita, ako sa popularita líši od $\mu_j$, cize variabilita popularity spevaka od jeho priemernej popularity. Táto variabilita je rovnaká (do poolu) pre všetkých spevákov, pozri - predpoklad
1. vrstva je vlastne ako sa modeluje popularita u jedného speváka
2. vrstva je ako sa to líši od priemeru celkovo
